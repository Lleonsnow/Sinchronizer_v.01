...
class Checker:
    def __init__(self, local, virtual, file_log, token, timer, logger):
        self.logger = logger
        self.params = {1: local, 2: virtual, 3: file_log, 4: token, 5: timer}

    def validate(self):
        if not os.path.exists(self.params[1]):
            self.logger.error(
                "[-] Путь к локальной директории должен быть строкой вида"
        ...
        if not self.params[2]:
            self.logger.error(
                "[-] Путь к директории на облачном хранилище должен быть строкой вида"
        ...
        if not self.params[5]:
            self.logger.error(
                "[-] Период для синхронизации должен быть целым числом в секундах [10] либо не указан"
            )
            return False
...
def search_difference(new, old) -> Optional[Tuple[Set[str], Set[str], Set[str]]]:
    ...
    result = [
        *reduce(
            lambda x, y: x + (y[:1] if y[0] not in difference_all else ()),
            intersection_files,
            (),
        )
    ]
...
def error_handler(logger, status_code: int) -> None:
    ...
    elif status_code == http.HTTPStatus.REQUEST_ENTITY_TOO_LARGE:
        logger.error("[-] " + errors["REQUEST_ENTITY_TOO_LARGE"])

    elif status_code in (http.HTTPStatus.SERVICE_UNAVAILABLE, http.HTTPStatus.LOCKED):
        logger.error("[-] " + errors["LOCKED"])
