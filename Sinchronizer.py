...
class Sinchronizer:
    def __init__(self, url: str, token: str, registration_tools,
                 dir_virtual: str, dir_local: str, timer) -> None:
        self.url = url
    ...
    def isinstance_virtual_dir(self) -> Response:
        while True:
            try:
                response = requests.get(url=self.url + self.links['resources'],
                                        headers=self.auth_headers,
                                        params=self.dir_check)
            except (ConnectionError, ReadTimeout):
                logger_01.error('[-] Отсутствует подключение к интернету, ожидаю соединения...')
                sleep(self.timer)
                continue
    ...
    def create_virtual_dir(self) -> bool:
        try:
            ...
            if response.status_code not in [200, 201, 202]:
                response = requests.put(url=self.url + self.links['resources'],
                                        headers=self.auth_headers,
                                        params=self.dir_check)

        except (ConnectionError, ReadTimeout):
            logger_01.error(f'[-] Отсутствует подключение к интернету.'
        ...
        if response.status_code == 201:
            logger_01.info(f'[+] Рабочая директория [{self.dir_virtual}] в облачном хранилище успешно создана')
            return True

        logger_01.info(f'[-] Рабочая директория [{self.dir_virtual}]'
    ...
    def load(self, path: str, files: Union[List, Set]) -> None:
        ...
        if self.isinstance_virtual_dir().status_code not in [200, 201, 202]:
            logger_01.info(f'[-] Отсутствует виртуальная директория [{self.dir_virtual}], приступаю к созданию')
        ...
        while head < tail:
            ...
            try:
                get_upload_link = requests.get(self.url + self.links['resources'] + self.links['upload'],
                                               headers=self.auth_headers,
                ...
                if get_upload_link.status_code in [200, 201, 202]:
                    ...
                    logger_01.info(f'[+] Файл [{current}] успешно загружен в облачное хранилище')
                ...
                logger_01.error(f'[-] Отсутствует подключение к интернету. Файл [{current}] не загружен')
                sleep(0.5)
            ...
            logger_01.info('[+] Файлы загружены в облачное хранилище, ожидаю синхронизацию...')
        ...
        while head < tail:
            ...
            try:
                ...
                if get_upload_link.status_code in [200, 201, 202]:
                    upload_url = get_upload_link.json()['href']
                    requests.post(url=upload_url, files=data)
                    logger_01.info(f'[+] Файл [{current}] успешно перезаписан в облачное хранилище')
                ...
                logger_01.error(f'[-] Отсутствует подключение к интернету. Файл [{current}] не перезаписан')
                sleep(0.5)
                logger_01.info('[+] Ожидаю следующего периода синхронизации...')
                sleep(self.timer)
                continue
            ...
            logger_01.info('[+] Файлы перезаписаны в облачное хранилище, ожидаю синхронизацию...')
    ...
    def delete(self, files: Set) -> None:
        ...
        while head < tail:
            ...
            try:
                response = requests.delete(self.url + self.links['resources'],
                                           headers=self.auth_headers,
                ...
                logger_01.error(f'[-] Отсутствует подключение к интернету. Файл [{file}] не был удален')
                sleep(0.5)
                logger_01.info('[+] Ожидаю соединения...')
                sleep(self.timer)
                continue
            ...
            logger_01.info('[+] Удаленные файлы удалены из облачного хранилища, ожидаю синхронизацию...')
    ...
    def get_info(self) -> Optional[Set[str]]:
        ...
        try:
            ...
            logger_01.error('[-] Отсутствует подключение к интернету. Данные не получены')
            sleep(1)
            logger_01.info('[+] Ожидаю следующего периода синхронизации...')
            return
    ...
    def monitor(self) -> None:
        while True:
            logger_01.info(f'[+] Программа начинает работу с директорией [{self.dir_local}]')
    ...
    def process_sinchronization(self, old_changes: Tuple):
        while True:
            ...
            if update:
                logger_01.info('[+] Обнаружены измененные файлы, перезаписываю в облачное хранилище')
                self.reload(self.dir_local, update)

            if new:
                logger_01.info('[+] Обнаружены новые файлы, загружаю в облачное хранилище')
                self.load(self.dir_local, new)

            if delete:
                logger_01.info('[+] Обнаружены удаленные файлы, удаляю в облачном хранилище')
                self.delete(delete)

            if not new and not delete and not update:
                logger_01.info('[-] Новых файлов или изменений не обнаружено, ожидаю синхронизацию...')
...
if __name__ == '__main__':
    ...
    lg.logger.remove(handler_id=0)
    logger_01 = lg.logger.bind(name='01')
    logger_02 = lg.logger.bind(name='02')

    logger_01.add(sink=sys.stdout,
                  level="DEBUG",
                  colorize=True,
    ...
    validate = service.Checker(dir_local_path, dir_virtual_path, file_log, Token, timeout, logger_01)
    is_launch = validate.validate()

    if not is_launch:
        sys.exit()

    sinchronizer = Sinchronizer(base_url, Token, req_tools,
                                dir_virtual_path,
                                dir_local_path,
                                int(timeout))
